...
const HorarioTable = () => {
 ...
 const renderProfessorAbreviado = (nome: string) => {
  ...
  if (partesNome.length >= 2) {
    ...
    const iniciaisRestanteNome = partesNome.slice(1).map((parte) => parte.charAt(0)).join("");
...
const renderizarTabela = () => {
  ...
  diasSemana.forEach((dia) => {
    const celulas = horarios.map((horario) => {
      ...
      const nomeProfessorAbreviado = aulaCorrespondente ? renderProfessorAbreviado(aulaCorrespondente.professor.nome) : "";
      // Abreviar a descrição do local
      const descricaoLocalAbreviada = aulaCorrespondente ? renderDescricaoLocalAbreviada(aulaCorrespondente.local.descricao) : "";

      // Renderizar as informações na célula
      return (
        <td key={`${dia}-${horario.inicio}`} className="info-cell">
          {nomeProfessorAbreviado}<br />
          {disciplinaAbreviada}<br />
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        {Array.isArray(periodosLetivos) && periodosLetivos.map(periodo => (
